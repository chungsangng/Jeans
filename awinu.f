...
c
      ...
      real lr,li,nu,alpha,amax,eps,fnu,dlr,dli
      real lr1,lr2,lr3,li1,li2,li3,wp,pi
      real lx(2),plx(3,2),yx(3),ftol,funk,emin
      integer i,j,k,nmax,nlr,nli,nmin,iter,itmax,inumax,iw
      common/para/ci,a0,nu,alpha,eps,nmin,nmax,amax
      namelist/in/lr,li,nu,fnu,dlr,dli,alpha,nmax,nmin,amax,eps,
     .  ftol,itmax,inumax,iw
c
      read(*,in)
      write(*,in)      
c
     ...
     .          " funk                    "
      do i = 0,inumax
        ...
        if (mod(i,iw).eq.0) then
          write(*,10)nu,plx(1,1),plx(1,2),nmax,nmin,yx(1)
        endif
        lr = plx(1,1)
        li = plx(1,2)
      enddo
