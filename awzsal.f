...
c
      complex pdf
      real lr,li,alpha,dlr,dli,fal
      real lx(2),plx(3,2),yx(3),ftol,funk
      real lrold,liold,alold,dlrda,dlida
      integer i,j,k,nlr,nli,iter,itmax,imode,ialmax,iw
      common/para/alpha
      namelist/in/lr,li,fal,dlr,dli,alpha,ftol,itmax,ialmax,iw
c
      read(*,in)
      write(*,in)
      write(*,5)" alpha                   ",
     ...
     .          " funk                    "
      ...
      write(*,10)alpha,plx(1,1),plx(1,2),yx(1)
      lr = plx(1,1)
      li = plx(1,2)
      lrold = lr
      liold = li
...
c
      omeg = cmplx(x(1),x(2))
      z = pdf(omeg)
      zerr = zabs((1.,0.) + alpha*((1.,0.)+omeg*z))
      if (zerr.eq.0.) then
        funk = -120.
      else
        funk = log(zerr)
